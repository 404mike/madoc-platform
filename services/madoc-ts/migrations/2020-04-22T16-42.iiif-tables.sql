...
begin
    ...
    insert into iiif_derivative (type, resource_id, context, created_by)
    values (canonical_resource_type, canonical_resource_id, target_context::ltree, added_by)
    on conflict do nothing;
...
begin
    ...
    if sid is not null then
        -- Derive Canvas.
        derived_id = derive_resource(canonical_id, sid, extra_context, added_by);
    end if;
...
begin
    ...
    if site_id is not null then
        ...
        raise debug 'Adding to site id %', site_urn;
        ...
        for key in 0..(array_length(canvas_ids, 1) - 1)
            loop
                ...
                if derived_canvas_id is null then
                    raise debug 'Did not found derived canvas, adding new one...';
                    -- We will get to this.
                    perform derive_resource(canvas_id, site_urn || '.manifest_' || derivative_id, added_by);
                end if;
        ...
        raise debug 'Found derived canvas_ids %', derived_canvas_ids;
        ...
        if array_length(derived_canvas_ids, 1) > 0 then
            ...
            raise debug 'Did not find any canvas ids.';
        end if;
    end if;
...
begin
    ...
    if site_id is not null then
        ...
        for key in 0..(array_length(manifest_ids, 1) - 1)
            loop
                ...
                if derived_manifest_id is null then
                    raise debug 'Did not found derived manifest, adding: %', manifest_id;
                    -- We will get to this.
                    perform derive_resource(manifest_id, site_urn || '.collection_' || derivative_id, added_by);
                end if;
        ...
        if array_length(derived_manifest_ids, 1) > 0 then
            ...
            raise debug 'Did not find any manifest ids.';
        end if;
    end if;
    ...
    foreach item in array input
        loop
            if (item ~ ('*.' || to_remove::text || '.*')::lquery) then
                ...
                raise debug 'nlevel % %, %', item, 1 + index(item, to_remove), nlevel(item);
...
begin
    ...
    raise debug 'Merging context 1: %', arr1;
    raise debug 'Merging context 2: %', arr2;
